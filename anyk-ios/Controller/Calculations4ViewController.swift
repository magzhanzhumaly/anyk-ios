...
class Calculations4ViewController: UIViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        var numberTemp = Int(popUpOption.components(separatedBy: CharacterSet.decimalDigits.inverted).joined())
        numberTemp = ((numberTemp ?? 0) % 10) + ((numberTemp ?? 0) / 100 % 10)
        
        let argument3: UILabel = {
            let label = UILabel()
            label.text = "Срок займа: \(numberTemp ?? 0) лет"
            label.font = .systemFont(ofSize: 14, weight: .medium)
            label.layer.masksToBounds = true
        ...
        let argument4: UILabel = {
            let label = UILabel()
            label.text = "У меня есть депозит в ЖССБ"
            label.font = .systemFont(ofSize: 14, weight: .medium)
            label.layer.masksToBounds = true
...
extension Calculations4ViewController: UITableViewDelegate, UITableViewDataSource {
